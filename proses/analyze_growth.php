...
$jenis_kelamin = strtoupper($bayi['jenis_kelamin']);
if ($jenis_kelamin !== 'L' && $jenis_kelamin !== 'P') {
    die('Jenis kelamin bayi tidak valid. Harus L atau P.');
}
...
$berat = isset($_POST['berat_badan']) ? floatval($_POST['berat_badan']) : 0;
$tinggi = isset($_POST['tinggi_badan']) ? floatval($_POST['tinggi_badan']) : 0;
$lingkar = isset($_POST['lingkar_kepala']) ? floatval($_POST['lingkar_kepala']) : 0;

$tanggal_input = date('Y-m-d');

// Simpan data pertumbuhan bayi ke database
...
// Kirim data ke backend Python Flask API untuk analisis
$payload = json_encode([
  'berat_badan' => $berat,
  'tinggi_badan' => $tinggi,
  'lingkar_kepala' => $lingkar,
  'jenis_kelamin' => $jenis_kelamin,
  'usia_bulan' => $usia_bulan // ← tambahkan ini
]);
...
$res = @file_get_contents('http://127.0.0.1:5000/predict', false, $ctx);

if ($res === false) {
    $knn = ['kategori' => 'Error analisis: Tidak bisa hubungi Flask API.'];
} else {
    $knn = json_decode($res, true);
    if (!isset($knn['kategori'])) {
        $knn = ['kategori' => 'Error analisis: Response tidak valid.'];
    }
}
